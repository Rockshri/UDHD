...
export function UserManagementPage(): JSX.Element {
  ...
  const onDeleteUser = async (target: UserRow): Promise<void> => {
    if (!window.confirm(`Delete user "${target.username}"? This cannot be undone.`)) return;
    setNotice(null);
    try {
      await deleteUser(target.userId).unwrap();
      setEditing((cur) => (cur?.userId === target.userId ? null : cur));
      setNotice({ kind: 'success', text: `${target.username} was deleted successfully.` });
    } catch (err) {
      setNotice({ kind: 'error', text: readError(err) });
    }
  };
    ...
    >
      <article className="space-y-4">
        ...
        {notice ? (
          <div
            className={cn(
              'rounded-lg border p-3 text-[12.5px] font-medium',
              notice.kind === 'success'
                ...
                <table className="w-full min-w-[960px] border-collapse text-[12.5px]">
                  ...
                  <tbody>
                    {rows.map((u, idx) => {
                      ...
                      const canDeleteRow = !isSelf && (me ? DELETABLE_ROLES[me.role].includes(u.role) : false);
